...
namespace LinLib
{
	...
	class LBPFeature
	{
		...
		inline static int CalcIndex(ushort p, ushort pa, ushort pb, ushort pc, ushort pd, ushort pe, ushort pf, ushort pg, ushort ph, int nan_value)
		{
			uchar a = 1, b = 2, c = 4, d = 8, e = 16, f = 32, g = 64, h = 128;
...
//						_pd[CalcIndex(_p[j], _p[j+1], _p2[j+1], _p2[j], _p2[j-1], _p[j-1], _p1[j-1], _p1[j], _p1[j+1], nan_value)]++;
	...
	public:
		/// Constructor
		LBPFeature(BorderType type = BORDER_PADD)
			: border_type(type) {}
